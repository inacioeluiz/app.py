...
# ==============================================
# 🤖 ARBITRAGEM AI — VERSÃO COM COMPROVANTE E APROVAÇÃO
# 💳 Pix + Upload de Comprovante + Aprovação Manual + WhatsApp
# ==============================================

ARQUIVO_USUARIOS = "usuarios.json"
ARQUIVO_PAGAMENTOS = "pagamentos.json"
ARQUIVO_COMPROVANTES = "comprovantes.json"
ADMIN_EMAIL = "suportearbitrageai@gmail.com"  # SEU E-MAIL DE ADMIN
SEU_WHATSAPP = "5521997524939"  # SEU NÚMERO COM DDD
...
def registrar_pagamento_pendente(email, plano, valor, id_transacao, comprovante_nome=""):
    ...
    usuarios[email]["plano_ativo"] = False  # Fica inativo até aprovação
    ...
    # Salva comprovante em lista separada para admin
...
def exibir_pagamento_pix(plano, email_cliente):
    ...
    if comprovante:
        ...
        # Salva o comprovante (simulação - em produção salvar em pasta)
...
if status_pagamento == "pendente":
    st.warning("⏳ **Pagamento pendente!** Seu plano será totalmente liberado assim que aprovarmos seu comprovante. Enviamos a mensagem no WhatsApp, aguarde nossa confirmação! 💬")

# ==============================================
# 🔐 PAINEL DE ADMINISTRAÇÃO — APROVAR PLANOS
# ==============================================
if user_email == ADMIN_EMAIL:
    ...
    if st.session_state.get("admin_mode", False):
        st.markdown("""
        <style>.stApp {background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 50%, #1a1a1a 100%);}</style>
        """, unsafe_allow_html=True)
            ...
            if not pendentes:
                ...
                for c in pendentes:
                    with st.expander(f"📌 {c['email']} — {c['plano']} — R$ {c['valor']:.2f}", expanded=True):
                        ...
                        with col_aprov:
                            if st.button(f"✅ APROVAR — {c['email']}", key=f"aprov_{c['id']}", type="primary"):
            ...
            for email, dados_user in usuarios.items():
                ...
                with st.expander(f"{status_icon} {email} — {dados_user.get('plano', 'Gratuito')}"):
                    ...
                    if not dados_user.get("plano_ativo", False):
                        if st.button(f"🔓 Ativar Plano Manualmente — {email}", key=f"ativar_{email}"):
...
# ==============================================
    ...
    if status_pagamento == "pendente":
        st.warning("""⏳ **Seu pagamento está aguardando aprovação!** 
        Assim que verificarmos seu comprovante, seu plano será totalmente liberado. 
        Envie o comprovante pelo WhatsApp para agilizar! 💬""")
